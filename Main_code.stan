functions{
 ...
 // }
  real my_custom_likelihood_lpdf(vector y, matrix x, matrix z, real alpha, real beta10, real beta20, int N, vector rho, vector beta1, vector beta2, vector phi, int[] K){
    int t = size(y);
    // int f = t/2;
    int f = t/2;
    vector[f] y1;
...
model{
  ...
  phi ~ multi_normal(mu, Sigma*D_W_inv);
  y ~ my_custom_likelihood(x, z, alpha, beta10, beta20, N, rho, beta1, beta2, phi, K); 
}
